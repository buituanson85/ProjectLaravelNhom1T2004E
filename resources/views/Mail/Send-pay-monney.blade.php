        ...
        <tr>
            <td>
                <table align="center" style="width:100%;max-width:670px;background:#ffffff;padding-top:40px;padding-left:40px;padding-right:40px;padding-bottom:40px">
                    <tbody>
                    <tr>
                        <td style="color:#0a73b7;font-family:Arial,sans-serif;font-size:24px;text-align:center;font-weight:bold">XÁC NHẬN RÚT TIỀN</td>
                    </tr>
                    <tr><td style="padding-bottom:30px;color:#f7a51c;font-family:Arial,sans-serif;font-size:18px;text-align:center;font-weight:bold;font-style:italic"></td>
                        ...
                        <td style="padding:0 10px 20px 10px;color:#373636;font-family:Arial,sans-serif;font-size:16px;line-height:20px"><p>Xin chào {{ $histories['name'] }}!</p><p>Lệnh rút tiền của bạn đã được xác nhận. bạn vui lòng kiểm tra lại tài khoản ngân hàng đã đăng ký!
                                <br><br>THÔNG TIN RÚT TIỀN.
                            </p>
                            <p style="color: red;font-size: 16px">{{ $histories['date'] }}</p>
                            <ul>
                                <li>Tên đối tác: <b>{{ $histories['name'] }}</b></li>
                                <li>Số tài khoản: <b>{{ $histories['account'] }}</b></li>
                                <li>Mã giao dịch: <b>{{ $histories['trading_code'] }}</b></li>
                                <li>Email: <b><a href="mailto:{{ $histories['email'] }}">{{ $histories['email'] }}</a></b></li>
                                <li>Số điện thoại: <b>{{ $histories['phone'] }}</b></li>
                                <li>Số tiền rút: <b>{{ $histories['monney_pay'] }}</b></li>
                                <li>Số tiền còn lại trong tài khoản: <b>{{ $histories['monney'] }}</b></li>
                            </ul>
                            <p>Nếu có phát sinh trong quá trình giao dịch, vui lòng
